...
-- Purpose-built people visibility for narrow operational roles.
drop policy if exists "asc3nd event staff read routed people" on asc3nd.people;
create policy "asc3nd event staff read routed people"
on asc3nd.people for select to authenticated
using (
  asc3nd_private.has_org_role(organization_id, array['event_staff'])
  and exists (
    select 1 from asc3nd.person_routes pr
    where pr.organization_id = people.organization_id
      and pr.person_id = people.id
      and pr.route_key in ('family','updates')
      and pr.status in ('new','active')
  )
);

drop policy if exists "asc3nd volunteers read assigned people" on asc3nd.people;
create policy "asc3nd volunteers read assigned people"
on asc3nd.people for select to authenticated
using (
  asc3nd_private.has_org_role(organization_id, array['volunteer'])
  and exists (
    select 1 from asc3nd.person_routes pr
    where pr.organization_id = people.organization_id
      and pr.person_id = people.id
...
create or replace function public.asc3nd_staff_route_queue(
  p_route_key text,
...
as $$
  select
    ...
    (
      ...
      where t.organization_id = p.organization_id and t.person_id = p.id
    ),
    coalesce((
      select cc.status
...
begin
  ...
  insert into asc3nd.person_memory(
    ...
    human_verified, sensitivity, occurred_at
  ) values (
    v_task.organization_id, v_task.person_id, 'followup_status',
    'Follow-up task status changed',
    jsonb_build_object('task_id', v_task.id, 'route_key', v_task.route_key, 'status', v_task.status),
    'followup_task:' || v_task.id::text,
    true, 'normal', now()
  );
