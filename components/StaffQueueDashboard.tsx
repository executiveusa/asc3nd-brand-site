...
function exportRows(rows: QueueRow[], route: string) {
  ...
  const url = URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`asc3nd-${route}-${new Date().toISOString().replaceAll(":","-").replace(".","-")}.csv`; a.style.display="none"; document.body.appendChild(a); a.click(); setTimeout(()=>{a.remove();URL.revokeObjectURL(url);},0);
}

export function StaffQueueDashboard() {
  const supabase = useMemo(() => createAsc3ndBrowserClient(), []);
  const [email,setEmail]=useState(""); const [signedIn,setSignedIn]=useState(false); const [route,setRoute]=useState<(typeof routes)[number][0]>("family"); const [rows,setRows]=useState<QueueRow[]>([]); const [loading,setLoading]=useState(false); const [exporting,setExporting]=useState(false); const [message,setMessage]=useState("");
  ...
  async function queuePage(nextRoute:string,offset:number){ return supabase.rpc("asc3nd_staff_queue",{p_route_key:nextRoute,p_limit:100,p_offset:offset}); }
  async function loadQueue(nextRoute=route){ setLoading(true); setMessage(""); const {data,error}=await queuePage(nextRoute,0); setRows((data as QueueRow[]|null)||[]); setMessage(error?error.message:""); setLoading(false); }
  async function exportAll(){ if(exporting)return; setExporting(true); setMessage("Preparing a fresh route export from Supabase…"); try { const all:QueueRow[]=[]; for(let offset=0;offset<10000;offset+=100){ const {data,error}=await queuePage(route,offset); if(error) throw error; const page=(data as QueueRow[]|null)||[]; all.push(...page); if(page.length<100) break; } if(!all.length){setMessage("There are no visible records to export for this route.");return;} if(all.length>=10000) throw new Error("Export safety limit reached. Narrow the data set before exporting."); exportRows(all,route); setMessage(`Exported ${all.length.toLocaleString()} current ${route} records.`); } catch(error){setMessage(error instanceof Error?`Export failed: ${error.message}`:"Export failed. Refresh and try again.");} finally{setExporting(false);} }
  ...
  return <div className={styles.dashboard}>
    <div className={styles.toolbar}><div className={styles.routes} role="tablist" aria-label="ICM relationship queues">{routes.map(([key,label])=><button key={key} type="button" role="tab" aria-selected={route===key} className={route===key?styles.activeRoute:undefined} onClick={()=>setRoute(key)}>{label}</button>)}</div><div className={styles.actions}><button type="button" onClick={()=>loadQueue()} disabled={loading}><RefreshCw size={16}/> Refresh</button><button type="button" onClick={exportAll} disabled={exporting}><Download size={16}/> {exporting?"Preparing…":"Export CSV"}</button><button type="button" onClick={signOut}><LogOut size={16}/> Sign out</button></div></div>
    {message?<p className={styles.notice}>{message}</p>:null}
    <div className={styles.queueHeader}><div><span>ICM route</span><h2>{routes.find(([key])=>key===route)?.[1]}</h2></div><strong>{loading?"Loading…":`${rows.length} latest visible`}</strong></div>
